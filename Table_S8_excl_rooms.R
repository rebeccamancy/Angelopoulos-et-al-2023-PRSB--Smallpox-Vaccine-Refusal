# Table_S8 - a four column table, deaths and death rate for WARDS both linear and non-linear models
# Producing alternative output without Rooms per dwelling
...
### Without average room
OLS_df <- read_excel(path = "Output_data/Results_Ward_OLS_without_rooms.xlsx")
fracreg_df <- read_excel(path = "Output_data/Results_Wards_fracreg_without_rooms.xlsx")
...
results_table$Variable_name <- factor(results_table$Variable_name,
                                      levels = c("(Intercept)","Smallpox","Smallpox x year","Year_Numeric",
                                                 "Population_density_1911","Perc_Irish_Born_1901"),
                                      labels = c("(Intercept)","Smallpox","Smallpox x year","Year",
                                                 "Population density","Percent Irish born"))
...
fileConn <- file("tables/Table_2_without_av_rooms.html", )
