...
# Map of smallpox deaths
COV_Yr <- subset(COV, Year == 1908)
wards_plus <- left_join(x=wards["Num"], y=COV_Yr, by=c("Num"="Municipal_Ward_Number"))
g_deaths <- ggplot(data=wards_plus) + geom_sf(aes(fill = Smallpox_deaths), colour = "lightgrey", size = 0.2) +
  geom_sf(data=subway, colour = "black", linetype = 6, size = 0.6) + 
  geom_sf(data=rivers, fill = "dodgerblue3", size = 0.2, colour="dodgerblue3") + 
  geom_sf(data=belvidere_smallpox, shape = 3, colour = "black", size = 2.5) +
...
left_panel <- plot_grid(g_deaths, g_cov, labels = c('A', 'B'), label_size = 12, ncol = 1)
right_panel <- g_time_series
fig_1 <- plot_grid(left_panel, right_panel, labels = c('', 'C'), label_size = 12, ncol = 2)
fig_1

ggsave(filename = paste0("fig_1_wards_deaths.png"), plot = fig_1, path = "figs", width = 31, height = 21, units = "cm")
ggsave(filename = paste0("fig_1_wards_deaths.pdf"), plot = fig_1, path = "figs", width = 31, height = 20, units = "cm")
